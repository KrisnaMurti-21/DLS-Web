    ...
    <div class="main-container d-flex">
        ...
        <?php endif; ?>
                <div class="shadow p-3 mb-5 bg-body-tertiary rounded">
                    ...
                    <p>Berikut adalah seluruh data siswa</p>
                    <br>
                    <div>
                            ...
                            <tbody>
                                ...
                                foreach ($result as $value) { ?>
                                    <tr>
                                        ...
                                        <td><?= $value['email']; ?></td>
                                        <td>
                                            <a href="<?= base_url('admin/viewSiswa/' . $value['idsiswa']) ?>"><button type="button" class="btn btn-success"><i class="bi bi-eye"></i></button></a>
                                            <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#editModal-<?= $value['idsiswa']; ?>"><i class="bi bi-pencil-square"></i></button>
                                            <a href="<?= base_url('/admin/removeSiswa/' . $value['idsiswa']) ?>" class="hapusSiswa"><button type="button" class="btn btn-danger"><i class="bi bi-trash-fill"></i></button></a>
                                        </td>
                                    </tr>
                                    <!-- Modal -->
                                    <div class="modal fade" id="editModal-<?= $value['idsiswa']; ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form id="editForm-<?= $value['idsiswa']; ?>" action="/admin/editSiswa/<?= $value['idsiswa']; ?>" method="post">
                                                        <div class="mb-3">
                                                            <label for="nama">Nama Siswa</label>
                                                            <input type="text" class="form-control" id="nama" name="nama" value="<?= $value['nama']; ?>" data-siswa-id="<?= $value['idsiswa']; ?>">
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="nis">NIS Siswa</label>
                                                            <input type="number" class="form-control" id="nis" name="nis" value="<?= $value['nis']; ?>" data-siswa-id="<?= $value['idsiswa']; ?>">
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="email">NIS Siswa</label>
                                                            <input type="text" class="form-control" id="email" name="email" value="<?= $value['email']; ?>" data-siswa-id="<?= $value['idsiswa']; ?>">
                                                        </div>
                                                    </form>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                    <button type="submit" form="editForm-<?= $value['idsiswa']; ?>" class="btn btn-primary">Save changes</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
